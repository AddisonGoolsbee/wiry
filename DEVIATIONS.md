...
| E11 | `flags` reads as a plain string (`"SA"`), not a rich flag object | scapy's `pkt.flags` compares equal to its integer, exposes per-bit attributes (`.S`, `.A`, `.SA`), is assignable (`pkt.flags.U = True`), supports `&=`/`\|=` with strings, and reprs as `<Flag 18 (SA)>`. Found by running scapy's own regression suite | OPEN |
| E12 | `Padding` is not excluded from enclosing length fields, and does not float to the end during assembly | scapy computes `IP()/Padding("abc")` as `len == 20` while the frame is 23 bytes, and assembles `Raw/Padding/Raw/Padding` as all Raw then all Padding. Found by running scapy's own regression suite | OPEN |
| E13 | Several scapy conveniences are absent: `str` payloads (`Raw("abc")`), integer layer indexing (`pkt[0]`), `getlayer` with field filters, address generators (`IP(dst=["a","b"])`, `Net`), and hostname resolution in address fields | Each is a distinct feature rather than a bug; listed together because scapy's suite exercises them heavily | OPEN |
