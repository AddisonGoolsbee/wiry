...
| E2 | IPv4/TCP options are parsed into a named list via `pkt[TCP].options`; the flat `options` field still returns the raw bytes (`raw_options()`) | Remaining: SACK blocks are not split into edge pairs, and options cannot yet be *written* back, so constructing a packet with options requires supplying the raw bytes | PARTIAL |
