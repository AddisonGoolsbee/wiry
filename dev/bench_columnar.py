...
def bench(path, limit):
    ...
    dt, want = timed(bulk)
    results["columns"] = row("wiry  columns()  one pass", dt, n, str(want))
    ...
    results["field_column"] = row("wiry  field_column() x4", dt, n)
    ...
    assert got == want, (got, want)
    results["loop"] = row("wiry  per-packet Python loop", dt, n)
...
def bench_filtered(path, limit):
    ...
    dt, hits = timed(bulk)
    row("wiry  columns(layer='TCP')", dt, n, f"({hits:,} tcp rows)")
    ...
    dt, got = timed(py_loop)
    assert got == hits
    row("wiry  per-packet Python loop", dt, n)
